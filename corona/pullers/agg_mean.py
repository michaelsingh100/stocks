...
class MeanPuller:
    def updatemean(self,start):
        results = ClosingPoints.objects.raw("select max(id), symbol, avg(price) as mean, std(price) as stan from ClosingPoints where date > '%s 00:00:00' group by symbol" % (start))
